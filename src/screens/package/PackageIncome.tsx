...
type Props = NativeStackScreenProps<RootStackParamList, "PackageIncome">;

const PackageIncome = memo(({ route }: Props) => {
  ...
  const onSubmit = async (values: ILoanProps) => {
    if (type === 1) {
      try {
        await TemplateApi.templatePostIncome({ template: template, incomeType: "Бэлэн" });
      } catch (err) {
        console.log(err);
      } finally {
        mutate("/goods/user");
    ...
    if (type === 2) {
      try {
        await TemplateApi.templatePostIncome({ template: template, incomeType: "Бэлэн бус" });
      } catch (err) {
        console.log(err);
      } finally {
        mutate("/goods/user");
    ...
    if (type === 3) {
      ...
      try {
        await TemplateApi.templatePostIncome({ loan: loan });
      } catch (err) {
        console.log(err);
      } finally {
        mutate("/goods/user");
  ...
  PackageIncome.displayName="PackageIncome";

export { PackageIncome };
